...
| F5 | MEDIO | Store carga toda la entidad en memoria por request | ✅ **RESUELTO + DESPLEGADO** — fase 1 (2026-07-05): filtro de igualdad + `id` en SQL + índice JSONB; fase 2 (2026-07-06): `ORDER BY` + `LIMIT` en SQL |
...
**Fase 2 (2026-07-06): `ORDER BY` + `LIMIT` empujados a SQL.** `readScoped` acepta `{sort, limit}`: el `LIMIT` va a SQL cuando hay límite finito y el sort es "SQL-seguro" o no hay sort; el `ORDER BY` solo se empuja para campos con forma de timestamp ISO (patrón `date`/`_date`/`_at` — ASCII y estructura fija, donde el orden de string jsonb coincide con el comparador JS bajo cualquier collation, con `NULLS FIRST/LAST` calcado a la regla null-primero-asc de `compareValues`). Campos sensibles a collation (`name`, `full_name`…) siguen ordenándose en JS y no truncan en SQL. Red de seguridad: el filtro/sort JS se re-aplica sobre la página y, si el re-filtro JS descarta filas de una página llena (el SQL puede sobre-emparejar filtros tipados), se re-consulta sin límite (camino exacto). Beneficio directo: los lookups `limit:1` (`findById`, email, membership) y los "últimos N por fecha" (`getLastInvoice`, listados) dejan de transferir la entidad entera.
**Verificado:** test diferencial contra Postgres 16 real (Docker) con dataset adversarial (nulls, campos ausentes, empates, filtros tipados, deriva de tipos `5` vs `"5"`, acentos): **539 combinaciones de filtro×sort×limit idénticas a la semántica JS de referencia**; sanity de rendimiento con 20k filas (limit 1 con sort: 6 ms vs 23 ms del fetch completo; PK: 1 ms); 12/12 tests, contratos multitenant/rbac/auth-storage, lint y build OK. Validado también en vivo contra el Postgres de producción tras el despliegue.
